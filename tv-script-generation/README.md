...
在本项目中，你将学会使用 RNNs 生成新的《辛普森一家》剧本。我们将会使用27季节目的部分剧本。

通过以下步骤生成你的 `jupyter notebook` 并运行第三个项目：
...
在 RussellCLoud 上创建名为`tv-script-generation`的项目，环境选择`keras`，然后在本地使用如下指令绑定它：

```
$ russell init tv-script-generation
```
...
russell run \
  --env keras \
  --data c482f8ea6b5442569d9bacf1dc6d7bab:data \
  --mode jupyter \
  --gpu
